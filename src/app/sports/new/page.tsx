...
export default function NewSportMatchPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Auto-register creator as host participant
    const teamSide = mode === 'department' ? 'host' : 'individual'
      ...
      <div className="card p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {(['department', 'individual'] as const).map(m => (
                <button key={m} type="button"
                  ...
                  className={`p-4 rounded-lg border-2 text-left transition-all ${
                    mode === m ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                  <div className="font-semibold text-sm text-gray-900">
                    {m === 'department' ? '🏫 학과 대항전' : '🙋 개인 매칭'}
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    {m === 'department' ? '학과 vs 학과 형식의 팀전' : '학과 무관 개인 참여'}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="form-group">
            ...
            <select id="sport_type" className="input-field"
              value={form.sport_type} onChange={e => set('sport_type', e.target.value)} required>
          ...
          {mode === 'department' && (
            <div className="grid grid-cols-2 gap-4">
              <div className="form-group">
                ...
                <select id="dept_host" className="input-field"
                  value={form.dept_host} onChange={e => set('dept_host', e.target.value)}>
              ...
              <div className="form-group">
                ...
                <div className="space-y-2">
                  <select id="dept_guest" className="input-field"
                    ...
                    <option value="">학과 선택</option>
                    {DEPARTMENTS.filter(d => d !== form.dept_host).map(d => (
                      <option key={d} value={d}>{d}</option>
                    ))}
                  </select>
                  <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                    <input type="checkbox"
                      checked={form.deptGuestOpen}
                      onChange={e => set('deptGuestOpen', e.target.checked)}
                      className="rounded" />
                    어느 학과든 모집
                  </label>
                </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="form-group">
              <label className="label" htmlFor="min_players">
                {mode === 'department' ? '팀당 최소 인원 *' : '최소 참여 인원 *'}
              </label>
              <input id="min_players" type="number" className="input-field"
                min={1} max={30}
            ...
            <div className="form-group">
              <label className="label" htmlFor="max_players">
                {mode === 'department' ? '팀당 최대 인원 *' : '최대 참여 인원 *'}
              </label>
              <input id="max_players" type="number" className="input-field"
                min={1} max={50}
          ...
          <div className="form-group">
            ...
            <textarea id="description" rows={3} className="input-field resize-none"
              placeholder="경기 방식, 준비물, 주의사항 등을 입력하세요"
