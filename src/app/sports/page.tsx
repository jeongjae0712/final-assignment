...
interface PageProps {
  searchParams: { sport?: string; mode?: string }
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <FilterLink href="/sports" active={!searchParams.sport && !searchParams.mode} label="전체" />
        <span className="text-gray-300 self-center">|</span>
        {SPORT_TYPES.map(sport => (
          <FilterLink key={sport} href={`/sports?sport=${sport}`} active={searchParams.sport === sport} label={sport} />
        ))}
        <span className="text-gray-300 self-center">|</span>
                ...
                {match.match_mode === 'individual' && (
                  <div className="mt-3">
                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                      <span>참여 인원</span><span>{totalCount} / {match.max_players}</span>
                    </div>
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-primary-500 rounded-full" style={{ width: `${Math.min((totalCount / match.max_players) * 100, 100)}%` }} />
                    </div>
                  </div>
                )}
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-gray-500 mb-0.5">
        <span className="truncate max-w-[100px]">{label}</span><span>{count}/{max}</span>
      </div>
      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
